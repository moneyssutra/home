        ...
        {tabs.map((tab) => {
          ...
          if (tab.id === "add") {
            return (
              ...
              >
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#00D09C] to-[#10B981] flex items-center justify-center shadow-lg shadow-[#00D09C]/30 transition-all hover:scale-105 active:scale-95">
                  <Plus className="h-7 w-7 text-white" strokeWidth={2.5} />
                </div>
                <span className="text-[10px] text-[#0B3D2E]/60 mt-1 font-medium">{tab.label}</span>
              </button>
            );
          }
            ...
            >
              <Icon 
                className={`h-6 w-6 mb-1 transition-colors ${
                  active ? "text-[#00D09C]" : "text-[#0B3D2E]/40"
                }`}
                strokeWidth={active ? 2.5 : 2}
              />
              <span className={`text-[10px] font-medium transition-colors ${
                active ? "text-[#00D09C]" : "text-[#0B3D2E]/60"
              }`}>
                {tab.label}
              </span>
            </button>
