...
async def get_basic_profile(request: Request):
    ...
    if not profile:
        # Return user info if no profile exists
        return {
            "name": user.get('name', ''),
            "email": user.get('email', ''),
            "mobile": '',
...
async def get_basic_profile_alt(request: Request):
    """Alias for /profile/basic - Get basic profile for current authenticated user"""
    user = await get_current_user(request)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
@api_router.post("/auth/sessions/logout")
...
# ============ REPORTS API ============
    ...
    # Monthly Income (sum of all recurring income)
    monthly_income = 0
    for inc in incomes:
    ...
    # Monthly Expense (last 3 month avg or current)
    monthly_expense = 0
    for exp in expenses:
    ...
    health_target = 1000000  # ₹10L recommended for metro
    ...
    recommended_equity = max(30, 100 - age)  # Minimum 30% equity
    ...
    required_corpus = annual_expense * 25  # 4% withdrawal rule
    
    # Retirement investments (NPS, EPF, PPF, long-term MF)
    retirement_corpus = 0
    for inv in investments:
        cat = inv.get('category', '').lower()
    ...
    # If no specific retirement tagged, use 30% of total investments
    ...
    # Weights: Emergency(20%), Insurance(15%), Savings(15%), Loan(15%), Credit(10%), Investment(15%), Retirement(10%)
