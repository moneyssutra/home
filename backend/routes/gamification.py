...
# Gender-friendly achievements - 30 Effective Badges in 8 Categories
# Bronze (starter), Silver (mid), Gold (advanced), Platinum (elite)
ACHIEVEMENTS = {
    ...
    "MONEYSUTRA_LEGEND": {"title": "MoneySutra Legend", "description": "All 30 badges, Level 20, Score 95+", "icon": "star", "xp_bonus": 500, "category": "elite", "tier": "platinum"},
}
...
async def process_gamification(request: Request):
    ...
    investments = await db.investments.find(user_filter, {"_id": 0, "name": 1, "currentValue": 1}).to_list(1000)
    ...
    checks = {
        ...
        "SCORE_100": score >= 100,
        # Behavior (4)
        "OVERSPEND_SLAYER": monthly_income > 0 and monthly_discretionary > 0 and (monthly_discretionary / monthly_income) < 0.15,
        "SMART_SPENDER": monthly_income > 0 and (monthly_discretionary / monthly_income) < 0.20,
        "DISCIPLINE_PRO": high_alerts == 0 and score >= 70,
        "BEHAVIORAL_CHAMPION": high_alerts == 0 and score >= 80,
        # Savings (4)
        "SAVED_10K": effective_funds >= 10000,
        "SAVED_1L": effective_funds >= 100000,
        "SAVINGS_RATE_30": savings_rate_pct >= 30,
        "CASH_FLOW_KING": savings_rate_pct >= 10 and effective_funds >= 100000,
        # Debt (3)
        "FIRST_EMI_CLOSED": total_debt == 0 and await db.loans.count_documents(user_filter) > 0,
        "DEBT_DESTROYER_50": total_debt > 0 and total_debt < effective_funds * 0.5,
        ...
        "INVESTMENT_STRATEGIST": monthly_income > 0 and sum(i.get("currentValue", 0) for i in investments) >= monthly_income * 12,
        # Streak (3)
        ...
        "MONEYSUTRA_LEGEND": pre_badge_level["level"] >= 20 and score >= 95 and current_achievements >= 29,
    }
