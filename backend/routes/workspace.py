"""Workspace routes - Full workspace management from server.py."""
...
from server_models import WorkspaceCreate, WorkspaceInvite, WorkspaceInviteByCode, ROLE_PERMISSIONS
from routes.auth import get_current_user
from routes.utils import get_user_workspace, ensure_user_has_workspace, check_permission
...
async def get_user_workspaces(request: Request):
    ...
    memberships = await db.workspace_members.find({"user_id": user_id, "status": "active"}, {"_id": 0}).to_list(100)
    workspaces = []
...
async def create_workspace(input: WorkspaceCreate, request: Request):
    ...
    workspace = {
        "id": workspace_id, "name": input.name, "type": input.type,
    ...
    return {
        "id": workspace_id, "name": input.name, "type": input.type,
        "owner_id": user_id, "invite_code": invite_code,
        "created_at": workspace["created_at"], "role": "owner"
    }
...
async def get_current_workspace(request: Request, workspace_id: Optional[str] = None):
    ...
    member_count = await db.workspace_members.count_documents({"workspace_id": workspace['id'], "status": "active"})
...
async def get_pending_invitations(request: Request):
    ...
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    user_email = user.get('email', '')
    invitations = await db.workspace_members.find({"user_email": user_email, "status": "pending"}, {"_id": 0}).to_list(100)
    result = []
...
async def join_workspace_by_code(invite: WorkspaceInviteByCode, request: Request):
    ...
    existing = await db.workspace_members.find_one({"workspace_id": workspace['id'], "user_id": user_id, "status": "active"}, {"_id": 0})
    if existing:
        raise HTTPException(status_code=400, detail="You are already a member of this workspace")
    pending = await db.workspace_members.find_one({"workspace_id": workspace['id'], "user_email": user_email, "status": "pending"}, {"_id": 0})
    if pending:
        await db.workspace_members.update_one({"id": pending['id']}, {"$set": {"user_id": user_id, "user_name": user_name, "status": "active", "accepted_at": datetime.now(timezone.utc).isoformat()}})
        return {"message": f"Joined workspace: {workspace['name']}", "workspace_id": workspace['id'], "role": pending['role']}
    member = {
        "id": f"wm_{uuid.uuid4().hex[:12]}", "workspace_id": workspace['id'],
        "user_id": user_id, "user_email": user_email, "user_name": user_name,
        "role": "viewer", "invited_by": None,
        "invited_at": datetime.now(timezone.utc).isoformat(),
        "accepted_at": datetime.now(timezone.utc).isoformat(), "status": "active"
    }
    await db.workspace_members.insert_one(member)
    return {"message": f"Joined workspace: {workspace['name']}", "workspace_id": workspace['id'], "role": "viewer"}
...
async def accept_workspace_invitation(member_id: str, request: Request):
    ...
    user_email = user.get('email', '')
    user_name = user.get('name', 'User')
    invitation = await db.workspace_members.find_one({"id": member_id, "user_email": user_email, "status": "pending"}, {"_id": 0})
    if not invitation:
        raise HTTPException(status_code=404, detail="Invitation not found or already accepted")
    await db.workspace_members.update_one({"id": member_id}, {"$set": {"user_id": user_id, "user_name": user_name, "status": "active", "accepted_at": datetime.now(timezone.utc).isoformat()}})
    workspace = await db.workspaces.find_one({"id": invitation['workspace_id']}, {"_id": 0})
    return {"message": f"Successfully joined {workspace['name'] if workspace else 'workspace'}", "workspace_id": invitation['workspace_id'], "role": invitation['role']}
...
async def invite_to_workspace(workspace_id: str, invite: WorkspaceInvite, request: Request):
    ...
    existing = await db.workspace_members.find_one({"workspace_id": workspace_id, "user_email": invite.email}, {"_id": 0})
    if existing:
    ...
    member = {
        ...
        "role": invite.role, "invited_by": user.get('user_id'),
        "invited_at": datetime.now(timezone.utc).isoformat(), "accepted_at": None, "status": "pending"
    }
    await db.workspace_members.insert_one(member)
    return {"message": f"Invitation sent to {invite.email}", "member_id": member['id'], "status": "pending"}
...
async def update_member_role(workspace_id: str, member_id: str, new_role: str, request: Request):
    ...
    await db.workspace_members.update_one({"id": member_id}, {"$set": {"role": new_role}})
...
async def remove_workspace_member(workspace_id: str, member_id: str, request: Request):
    ...
    await db.workspace_members.update_one({"id": member_id}, {"$set": {"status": "removed"}})
...
async def regenerate_invite_code(workspace_id: str, request: Request):
    ...
    await db.workspaces.update_one({"id": workspace_id}, {"$set": {"invite_code": new_code}})
